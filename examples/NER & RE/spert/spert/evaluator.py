...
class Evaluator:
    def __init__(self, dataset: Dataset, input_reader: BaseInputReader, text_encoder: BertTokenizer,
        ...
        self._pred_entities_scores = []  # prediction
        self._pred_relations_scores = []  # prediction
    ...
    def eval_batch(self, batch_entity_clf: torch.tensor, batch_rel_clf: torch.tensor,
        ...
        batch_pred_entities, batch_pred_relations,batch_pred_entities_scores ,batch_pred_relations_scores = prediction.convert_predictions(batch_entity_clf, batch_rel_clf,
                                                                                   batch_rels, batch,
        ...
        self._pred_entities_scores.extend(batch_pred_entities_scores)
        self._pred_relations_scores.extend(batch_pred_relations_scores)
    ...
    def store_predictions(self):
        prediction.store_predictions(self._dataset.documents, self._pred_entities,
                                     self._pred_relations, self._predictions_path,
                                      self._pred_entities_scores,  self._pred_relations_scores)
    def store_predictions2(self):
        prediction.store_predictions2(self._dataset.documents, self._pred_entities,
                                     self._pred_relations, self._predictions_path)
    ...
    def _convert_pred(self, docs: List[Document]):
        for doc in docs:
            gt_relations = doc.relations
            gt_entities = doc.entities
            ...
            self._pred_entities.append(sample_gt_entities)
