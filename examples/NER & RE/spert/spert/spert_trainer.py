...
class SpERTTrainer(BaseTrainer):
    ...
    def _predict(self, model: torch.nn.Module, dataset: Dataset, input_reader: BaseInputReader):
        ...
        pred_entities = []
        pred_relations = []
        pred_entities_scores = []
        pred_relations_scores = []
        ...
        with torch.no_grad():
            ...
            for batch in tqdm(data_loader, total=total, desc='Predict'):
                ...
                predictions = prediction.convert_predictions(entity_clf, rel_clf, rels,
                                                             ...
                                                             input_reader)
                batch_pred_entities, batch_pred_relations,batch_pred_entities_scores,batch_pred_relations_scores = predictions

                
                pred_entities.extend(batch_pred_entities)
                pred_relations.extend(batch_pred_relations)
                pred_entities_scores.extend(batch_pred_entities_scores)
                pred_relations_scores.extend(batch_pred_relations_scores)
        #print()
        prediction.store_predictions(dataset.documents, pred_entities, pred_relations, self._args.predictions_path, pred_entities_scores,pred_relations_scores)
