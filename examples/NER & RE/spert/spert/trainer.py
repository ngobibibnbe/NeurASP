...
class BaseTrainer:
    ...
    def __init__(self, args: argparse.Namespace):
        ...
        if hasattr(args, 'save_path'):
            self._save_path = os.path.join(self._args.save_path, self._args.label, str(self._args.rel_filter_threshold),str(self._args.epochs))
            util.create_directories_dir(self._save_path)

        # logging
        if hasattr(args, 'log_path'):
            self._log_path = os.path.join(self._args.log_path, self._args.label, str(self._args.rel_filter_threshold),str(self._args.epochs))
